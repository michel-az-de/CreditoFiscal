...
- **Segregação de interface (ISP)** — em vez de um `IBarramentoDeMensagens` único, há `IMensagemPublisher` e `IMensagemConsumer` separados (o controller só publica, o consumer só consome). `AdaptadorRabbitMq` implementa as duas, e o DI registra **a mesma instância singleton** para ambas.
